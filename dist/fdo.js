...
function propagator_divStep(space, config, varIndex1, varIndex2, varIndex3) {
  fdlib.ASSERT(varIndex1 >= 0 && varIndex2 >= 0 && varIndex3 >= 0, 'expecting three vars', varIndex1, varIndex2, varIndex3);
  ...
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log('propagator_divStep; indexes:', varIndex1, varIndex2, varIndex3, 'doms:', fdlib.domain__debug(domain1), 'div', fdlib.domain__debug(domain2), 'was', fdlib.domain__debug(domain3), 'now', fdlib.domain__debug(space.vardoms[varIndex3]));
  });
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex1], true, fdlib.domain__debug);
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex2], true, fdlib.domain__debug);
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex3], true, fdlib.domain__debug);
}
/**
...
function _propagator_divStep(domain1, domain2, domResult) {
  fdlib.ASSERT_NORDOM(domain1);
  fdlib.ASSERT_NORDOM(domain2);
  fdlib.ASSERT_NORDOM(domResult);
  fdlib.ASSERT(domain1 && domain2 && domResult, 'SHOULD_NOT_BE_REJECTED');
  var domain = fdlib.domain_divby(domain1, domain2);
  return fdlib.domain_intersection(domResult, domain);
}
...
function markov_getNextRowToSolve(space, matrix) {
  ...
  for (var i = 0; i < matrix.length; i++) {
    ...
    if (boolDomain === undefined || fdlib.domain_getValue(boolDomain) === 1) {
      break;
    }
  }
...
function markov_mergeDomainAndLegend(inputLegend, domain) {
  ...
  var listed = fdlib.domain_toList(domain);
...
function markov_createProbVector(space, matrix, expandVectorsWith, valueCount) {
  ...
  if (!probVector || probVector.length !== valueCount) {
    fdlib.THROW('E_EACH_MARKOV_VAR_MUST_HAVE_PROB_VECTOR_OR_ENABLE_EXPAND_VECTORS');
  }
...
function propagator_markovStepBare(space, config, varIndex) {
  ...
  fdlib.ASSERT(typeof varIndex === 'number', 'VAR_INDEX_SHOULD_BE_NUMBER');
  var domain = space.vardoms[varIndex];
  fdlib.ASSERT_NORDOM(domain);
  fdlib.ASSERT(domain, 'SHOULD_NOT_BE_REJECTED');

  if (!fdlib.domain_isSolved(domain)) {
    fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
      return log('propagator_markovStepBare; indexes:', varIndex, 'was solved:', fdlib.domain__debug(domain));
    });
    return;
  }

  var value = fdlib.domain_min(domain); // Note: solved so lo=hi=value

  var configVarDistOptions = config.varDistOptions;
  var distributeOptions = configVarDistOptions[config.allVarNames[varIndex]];
  fdlib.ASSERT(distributeOptions, 'var should have a config', varIndex, distributeOptions && JSON.stringify(configVarDistOptions));
  fdlib.ASSERT(distributeOptions.valtype === 'markov', 'var should be a markov var', distributeOptions.valtype);
  var expandVectorsWith = distributeOptions.expandVectorsWith;
  fdlib.ASSERT(distributeOptions.matrix, 'there should be a matrix available for every var');
  fdlib.ASSERT(distributeOptions.legend || expandVectorsWith !== null, 'every var should have a legend or expandVectorsWith set'); // Note: expandVectorsWith can be 0, so check with null
  ...
  if (pos < 0 || pos >= probabilities.length || probabilities[pos] === 0) {
    space.vardoms[varIndex] = fdlib.domain_createEmpty();
  }

  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log('propagator_markovStepBare; indexes:', varIndex, 'was:', fdlib.domain__debug(domain), 'became:', fdlib.domain__debug(space.vardoms[varIndex]));
  });
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex], true, fdlib.domain__debug);
}
...
function propagator_minStep(space, config, varIndex1, varIndex2, varIndex3) {
  ...
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log('propagator_minStep; indexes:', varIndex1, varIndex2, varIndex3, 'doms:', fdlib.domain__debug(domain1), fdlib.domain__debug(domain2), 'was', fdlib.domain__debug(domain3), 'now', fdlib.domain__debug(space.vardoms[varIndex3]));
  });
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex1], true, fdlib.domain__debug);
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex2], true, fdlib.domain__debug);
  fdlib.ASSERT(fdlib.domain_isEmpty(nR) || fdlib.ASSERT_NORDOM(nR, true, fdlib.domain__debug) || true, 'R can be empty');
}
/**
...
function _propagator_minStep(domain1, domain2, domResult) {
  fdlib.ASSERT_NORDOM(domain1);
  fdlib.ASSERT_NORDOM(domain2);
  fdlib.ASSERT(domain1 && domain2, 'SHOULD_NOT_BE_REJECTED');
  var domain = fdlib.domain_minus(domain1, domain2);

  if (!domain) {
    fdlib.TRACE('_propagator_minStep resulted in empty domain');
    return domain;
  }
...
function propagator_mulStep(space, config, varIndex1, varIndex2, varIndex3) {
  ...
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log('propagator_mulStep; indexes:', varIndex1, varIndex2, varIndex3, 'doms:', fdlib.domain__debug(domain1), 'mul', fdlib.domain__debug(domain2), 'was', fdlib.domain__debug(domain3), 'now', fdlib.domain__debug(vardoms[varIndex3]));
  });
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex1], true, fdlib.domain__debug);
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex2], true, fdlib.domain__debug);
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex3], true, fdlib.domain__debug);
...
function _propagator_mulStep(domain1, domain2, domResult) {
  ...
  fdlib.ASSERT_NORDOM(domain2);
  fdlib.ASSERT(domain1 && domain2, 'SHOULD_NOT_BE_REJECTED');
  var domain = fdlib.domain_mul(domain1, domain2);
  return fdlib.domain_intersection(domResult, domain);
}
...
function propagator_reifiedStepBare(space, config, leftVarIndex, rightVarIndex, resultVarIndex, opFunc, nopFunc, opName, invOpName, opRejectChecker, nopRejectChecker) {
  fdlib.ASSERT(space._class === '$space', 'SPACE_SHOULD_BE_SPACE');
  fdlib.ASSERT(typeof leftVarIndex === 'number', 'VAR_INDEX_SHOULD_BE_NUMBER');
  fdlib.ASSERT(typeof rightVarIndex === 'number', 'VAR_INDEX_SHOULD_BE_NUMBER');
  fdlib.ASSERT(typeof resultVarIndex === 'number', 'VAR_INDEX_SHOULD_BE_NUMBER');
  fdlib.ASSERT(typeof opName === 'string', 'OP_SHOULD_BE_STRING');
  fdlib.ASSERT(typeof invOpName === 'string', 'NOP_SHOULD_BE_STRING');
  var vardoms = space.vardoms;
  var domResult = vardoms[resultVarIndex];
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log('propagator_reifiedStepBare before; op:', opName, ', indexes:', resultVarIndex, '=', leftVarIndex, opName + '?', rightVarIndex, ', doms:', fdlib.domain__debug(vardoms[resultVarIndex]), '=', fdlib.domain__debug(vardoms[leftVarIndex]), opName + '?', fdlib.domain__debug(vardoms[rightVarIndex]));
  ...
  if (fdlib.domain_isZero(domResult)) {
    nopFunc(space, config, leftVarIndex, rightVarIndex);
  } else if (fdlib.domain_hasNoZero(domResult)) {
    ...
    fdlib.ASSERT_NORDOM(domain2);
    fdlib.ASSERT(domain1 && domain2, 'SHOULD_NOT_BE_REJECTED');
    fdlib.ASSERT(!fdlib.domain_isZero(domResult) && !fdlib.domain_hasNoZero(domResult), 'result should be booly now');

    if (nopRejectChecker(domain1, domain2)) {
      fdlib.ASSERT(!opRejectChecker(domain1, domain2), 'with non-empty domains op and nop cant BOTH reject');
      vardoms[resultVarIndex] = fdlib.domain_removeValue(domResult, 0);
      opFunc(space, config, leftVarIndex, rightVarIndex);
    } else if (opRejectChecker(domain1, domain2)) {
      vardoms[resultVarIndex] = fdlib.domain_removeGtUnsafe(domResult, 0);
      nopFunc(space, config, leftVarIndex, rightVarIndex);
    }
  }

  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log('propagator_reifiedStepBare after; doms:', fdlib.domain__debug(vardoms[resultVarIndex]), '=', fdlib.domain__debug(vardoms[leftVarIndex]), opName + '?', fdlib.domain__debug(vardoms[rightVarIndex]));
  });
  fdlib.ASSERT_NORDOM(space.vardoms[leftVarIndex], true, fdlib.domain__debug);
  fdlib.ASSERT_NORDOM(space.vardoms[rightVarIndex], true, fdlib.domain__debug);
  fdlib.ASSERT_NORDOM(space.vardoms[resultVarIndex], true, fdlib.domain__debug);
}
...
function propagator_ringStepBare(space, config, varIndex1, varIndex2, varIndex3, opName, opFunc) {
  ...
  var domain2 = vardoms[varIndex2];
  var domain3 = vardoms[varIndex3];
  fdlib.ASSERT(opName === 'plus' && opFunc === fdlib.domain_plus || opName === 'min' && opFunc === fdlib.domain_minus || opName === 'mul' && opFunc === fdlib.domain_mul || opName === 'div' && opFunc === fdlib.domain_invMul, 'should get proper opfunc');
  ...
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log(' - doms before:', fdlib.domain__debug(domain3), '=', fdlib.domain__debug(domain1), {
      u: '+',
      n: '-',
      l: '*',
      v: '/'
    }[opName[2]], fdlib.domain__debug(domain2));
  });
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log(' - doms after :', fdlib.domain__debug(vardoms[varIndex3]), '=', fdlib.domain__debug(vardoms[varIndex1]), {
      u: '+',
      n: '-',
      l: '*',
      v: '/'
    }[opName[2]], fdlib.domain__debug(vardoms[varIndex2]));
  });
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex1], true, fdlib.domain__debug);
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex2], true, fdlib.domain__debug);
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex3], true, fdlib.domain__debug);
...
function _propagator_ringStepBare(domain1, domain2, domainResult, opFunc, opName) {
  fdlib.ASSERT(typeof opFunc === 'function', 'EXPECTING_FUNC_TO_BE:', opName);
  fdlib.ASSERT_NORDOM(domain1);
  fdlib.ASSERT_NORDOM(domain2);
  fdlib.ASSERT(domain1 && domain2, 'SHOULD_NOT_BE_REJECTED');
  var domain = opFunc(domain1, domain2);
  return fdlib.domain_intersection(domainResult, domain);
}
...
function propagator_ltStepBare(space, config, varIndex1, varIndex2) {
  fdlib.ASSERT(space._class === '$space', 'SHOULD_GET_SPACE');
  fdlib.ASSERT(typeof varIndex1 === 'number', 'VAR_INDEX_SHOULD_BE_NUMBER');
  fdlib.ASSERT(typeof varIndex2 === 'number', 'VAR_INDEX_SHOULD_BE_NUMBER');
  var domain1 = space.vardoms[varIndex1];
  var domain2 = space.vardoms[varIndex2];
  fdlib.ASSERT_NORDOM(domain1);
  fdlib.ASSERT_NORDOM(domain2);
  fdlib.ASSERT(domain1 && domain2, 'SHOULD_NOT_BE_REJECTED');
  var lo1 = fdlib.domain_min(domain1);
  var hi2 = fdlib.domain_max(domain2);
  space.vardoms[varIndex1] = fdlib.domain_removeGte(domain1, hi2);
  space.vardoms[varIndex2] = fdlib.domain_removeLte(domain2, lo1);
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log('propagator_ltStepBare; indexes:', varIndex1, varIndex2, ', from:', fdlib.domain__debug(domain1), '<', fdlib.domain__debug(domain2), ', to:', fdlib.domain__debug(space.vardoms[varIndex1]), '<', fdlib.domain__debug(space.vardoms[varIndex2]));
  });
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex1], true, fdlib.domain__debug);
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex2], true, fdlib.domain__debug);
}
...
function propagator_ltStepWouldReject(domain1, domain2) {
  fdlib.ASSERT_NORDOM(domain1);
  fdlib.ASSERT_NORDOM(domain2);
  fdlib.ASSERT(domain1 && domain2, 'NON_EMPTY_DOMAIN_EXPECTED');
  var result = fdlib.domain_min(domain1) >= fdlib.domain_max(domain2);
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log('propagator_ltStepWouldReject;', fdlib.domain__debug(domain1), '>=?', fdlib.domain__debug(domain2), '=>', fdlib.domain_min(domain1), '>=?', fdlib.domain_max(domain2), '->', result);
  });
  return result;
}
/**
...
function propagator_lteStepBare(space, config, varIndex1, varIndex2) {
  ...
  var lo1 = fdlib.domain_min(domain1);
  var hi2 = fdlib.domain_max(domain2);
  space.vardoms[varIndex1] = fdlib.domain_removeGte(domain1, hi2 + 1);
  space.vardoms[varIndex2] = fdlib.domain_removeLte(domain2, lo1 - 1);
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log('propagator_ltStepBare; indexes:', varIndex1, varIndex2, ', from:', fdlib.domain__debug(domain1), '<=', fdlib.domain__debug(domain2), ', to:', fdlib.domain__debug(space.vardoms[varIndex1]), '<=', fdlib.domain__debug(space.vardoms[varIndex2]));
  });
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex1], true, fdlib.domain__debug);
...
function propagator_lteStepWouldReject(domain1, domain2) {
  ...
  var result = fdlib.domain_min(domain1) > fdlib.domain_max(domain2);
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log('propagator_lteStepWouldReject;', fdlib.domain__debug(domain1), '>?', fdlib.domain__debug(domain2), '=>', fdlib.domain_min(domain1), '>?', fdlib.domain_max(domain2), '->', result);
  });
  return result;
}
/**
...
function propagator_eqStepBare(space, config, varIndex1, varIndex2) {
  ...
  fdlib.ASSERT_NORDOM(domain2);
  fdlib.ASSERT(domain1 && domain2, 'SHOULD_NOT_BE_REJECTED');
  var result = fdlib.domain_intersection(domain1, domain2);
  vardoms[varIndex1] = result;
  vardoms[varIndex2] = result;
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log('propagator_eqStepBare; indexes:', varIndex1, varIndex2, 'doms:', fdlib.domain__debug(domain1), 'eq', fdlib.domain__debug(domain2), '->', fdlib.domain__debug(result));
  });
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex1], true, fdlib.domain__debug);
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex2], true, fdlib.domain__debug);
}
...
function propagator_eqStepWouldReject(domain1, domain2) {
  ...
  fdlib.ASSERT_NORDOM(domain2);
  fdlib.ASSERT(domain1 && domain2, 'NON_EMPTY_DOMAIN_EXPECTED');
  var result = fdlib.domain_sharesNoElements(domain1, domain2);
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log('propagator_eqStepWouldReject;', fdlib.domain__debug(domain1), '!==', fdlib.domain__debug(domain2), '->', result);
  });
  return result;
}
...
function propagator_neqStepBare(space, config, varIndex1, varIndex2) {
  fdlib.ASSERT(space && space._class === '$space', 'SHOULD_GET_SPACE');
  fdlib.ASSERT(typeof varIndex1 === 'number', 'VAR_INDEX_SHOULD_BE_NUMBER');
  fdlib.ASSERT(typeof varIndex2 === 'number', 'VAR_INDEX_SHOULD_BE_NUMBER');
  var vardoms = space.vardoms;
  var domain1 = vardoms[varIndex1];
  ...
  fdlib.ASSERT_NORDOM(domain1);
  fdlib.ASSERT_NORDOM(domain2);
  fdlib.ASSERT(domain1 && domain2, 'SHOULD_NOT_BE_REJECTED'); // Remove solved value from the other domain. confirm neither rejects over it.

  var value = fdlib.domain_getValue(domain1);

  if (value === fdlib.NO_SUCH_VALUE) {
    // Domain1 is not solved, remove domain2 from domain1 if domain2 is solved
    value = fdlib.domain_getValue(domain2);

    if (value !== fdlib.NO_SUCH_VALUE) {
      vardoms[varIndex1] = fdlib.domain_removeValue(domain1, value);
    }
  } else if (domain1 === domain2) {
    vardoms[varIndex1] = fdlib.domain_createEmpty();
    vardoms[varIndex2] = fdlib.domain_createEmpty();
  } else {
    vardoms[varIndex2] = fdlib.domain_removeValue(domain2, value);
  }

  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log('propagator_neqStepBare; indexes:', varIndex1, varIndex2, 'doms:', fdlib.domain__debug(domain1), 'neq', fdlib.domain__debug(domain2), '->', fdlib.domain__debug(vardoms[varIndex1]), fdlib.domain__debug(vardoms[varIndex2]));
  });
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex1], true, fdlib.domain__debug);
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex2], true, fdlib.domain__debug);
}
...
function propagator_neqStepWouldReject(domain1, domain2) {
  var value = fdlib.domain_getValue(domain1);
  var result = value !== fdlib.NO_SUCH_VALUE && value === fdlib.domain_getValue(domain2);
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_PROPSTEPS, function (log) {
    return log('propagator_neqStepWouldReject;', fdlib.domain__debug(domain1), '===', fdlib.domain__debug(domain2), '->', result);
  });
  return result;
}
...
function propagator_addReified(config, opname, leftVarIndex, rightVarIndex, resultVarIndex) {
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(typeof opname === 'string', 'OP_SHOULD_BE_STRING');
  fdlib.ASSERT(typeof leftVarIndex === 'number' && leftVarIndex >= 0, 'LEFT_VAR_SHOULD_BE_VALID_INDEX', leftVarIndex);
  fdlib.ASSERT(typeof rightVarIndex === 'number' && rightVarIndex >= 0, 'RIGHT_VAR_SHOULD_BE_VALID_INDEX', rightVarIndex);
  fdlib.ASSERT(typeof resultVarIndex === 'number' && resultVarIndex >= 0, 'RESULT_VAR_SHOULD_BE_VALID_INDEX', resultVarIndex);
  var nopName;
  var opFunc;
  var nopFunc;
  var opRejectChecker;
  ...
  switch (opname) {
    ...
    default:
      fdlib.THROW('UNKNOWN_REIFIED_OP');
  }
...
function propagator_addRingPlusOrMul(config, targetOpName, invOpName, opFunc, nopFunc, leftVarIndex, rightVarIndex, resultVarIndex) {
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(typeof targetOpName === 'string', 'OP_SHOULD_BE_STRING');
  fdlib.ASSERT(typeof invOpName === 'string', 'INV_OP_SHOULD_BE_STRING');
  fdlib.ASSERT(typeof leftVarIndex === 'number' && leftVarIndex >= 0, 'LEFT_VAR_SHOULD_BE_VALID_INDEX', leftVarIndex);
  fdlib.ASSERT(typeof rightVarIndex === 'number' && rightVarIndex >= 0, 'RIGHT_VAR_SHOULD_BE_VALID_INDEX', rightVarIndex);
  fdlib.ASSERT(typeof resultVarIndex === 'number' && resultVarIndex >= 0, 'RESULT_VAR_SHOULD_BE_VALID_INDEX', resultVarIndex);
...
function propagator_addRing(config, A, B, C, opName, opFunc) {
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(typeof A === 'number' && A >= 0, 'LEFT_VAR_SHOULD_BE_VALID_INDEX', A);
  fdlib.ASSERT(typeof B === 'number' && B >= 0, 'RIGHT_VAR_SHOULD_BE_VALID_INDEX', B);
  fdlib.ASSERT(typeof C === 'number' && C >= 0, 'RESULT_VAR_SHOULD_BE_VALID_INDEX', C);
...
function propagator_addPlus(config, leftVarIndex, rightVarIndex, resultVarIndex) {
  propagator_addRingPlusOrMul(config, 'plus', 'min', fdlib.domain_plus, fdlib.domain_minus, leftVarIndex, rightVarIndex, resultVarIndex);
}
/**
 * @param {$config} config
 * @param {number} leftVarIndex
...
function propagator_addRingMul(config, leftVarIndex, rightVarIndex, resultVarIndex) {
  propagator_addRingPlusOrMul(config, 'mul', 'div', fdlib.domain_mul, fdlib.domain_invMul, leftVarIndex, rightVarIndex, resultVarIndex);
}
/**
...
function propagator_addSum(config, varIndexes, resultVarIndex) {
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(Array.isArray(varIndexes), 'varIndexes should be an array of var names', varIndexes);
  fdlib.ASSERT(typeof resultVarIndex === 'number' && resultVarIndex >= 0, 'RESULT_VAR_SHOULD_BE_VALID_INDEX', typeof resultVarIndex, resultVarIndex);
  var len = varIndexes.length;

  switch (len) {
    case 0:
      fdlib.THROW('SUM_REQUIRES_VARS');
      return undefined;
  ...
  fdlib.ASSERT(len > 2, 'expecting at least 3 elements in the list...', varIndexes);
  var t1;
...
function propagator_addProduct(config, varIndexes, resultVarIndex) {
  ...
  switch (varIndexes.length) {
    case 0:
      fdlib.THROW('PRODUCT_REQUIRES_VARS');
      return undefined;
...
function propagator_addMarkov(config, varIndex) {
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(typeof varIndex === 'number' && varIndex >= 0, 'VAR_SHOULD_BE_VALID_INDEX', varIndex);
...
function config_create() {
  var config = {
    ...
    _varNamesTrie: fdlib.trie_create(),
...
function config_clone(config, newDomains) {
  ...
  var clone = {
    _class: '$config',
    _varNamesTrie: fdlib.trie_create(allVarNames),
    ...
    initialDomains: newDomains ? newDomains.map(fdlib.domain_toSmallest) : initialDomains,
    // <varName:domain>
...
function config_addVarNothing(config, varName) {
  return _config_addVar(config, varName, fdlib.domain_createRange(fdlib.SUB, fdlib.SUP));
}
/**
 * @param {$config} config
 * @param {number} lo
...
function config_addVarAnonRange(config, lo, hi) {
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(typeof lo === 'number', 'A_LO_MUST_BE_NUMBER');
  fdlib.ASSERT(typeof hi === 'number', 'A_HI_MUST_BE_NUMBER');
...
function config_addVarRange(config, varName, lo, hi) {
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(typeof varName === 'string' || varName === true, 'A_VARNAME_SHOULD_BE_STRING_OR_TRUE');
  fdlib.ASSERT(typeof lo === 'number', 'A_LO_MUST_BE_NUMBER');
  fdlib.ASSERT(typeof hi === 'number', 'A_HI_MUST_BE_NUMBER');
  fdlib.ASSERT(lo <= hi, 'A_RANGES_SHOULD_ASCEND');
  var domain = fdlib.domain_createRange(lo, hi);
...
function config_addVarDomain(config, varName, domain, _allowEmpty, _override) {
  fdlib.ASSERT(Array.isArray(domain), 'DOMAIN_MUST_BE_ARRAY_HERE');
  return _config_addVar(config, varName, fdlib.domain_anyToSmallest(domain), _allowEmpty, _override);
}
/**
 * @param {$config} config
 * @param {number} value
...
function config_addVarAnonConstant(config, value) {
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(typeof value === 'number', 'A_VALUE_SHOULD_BE_NUMBER');
...
function config_addVarConstant(config, varName, value) {
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(typeof varName === 'string' || varName === true, 'varName must be a string or true for anon');
  fdlib.ASSERT(typeof value === 'number', 'A_VALUE_SHOULD_BE_NUMBER');
  var domain = fdlib.domain_createRange(value, value);
  return _config_addVar(config, varName, domain);
}
/**
 * @param {$config} config
...
function _config_addVar(config, varName, domain, _allowEmpty, _override) {
  ...
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(_allowEmpty || domain, 'NON_EMPTY_DOMAIN');
  fdlib.ASSERT(_allowEmpty || fdlib.domain_min(domain) >= fdlib.SUB, 'domain lo should be >= SUB', domain);
  fdlib.ASSERT(_allowEmpty || fdlib.domain_max(domain) <= fdlib.SUP, 'domain hi should be <= SUP', domain);

  if (_override) {
    fdlib.ASSERT(fdlib.trie_has(config._varNamesTrie, varName), 'Assuming var exists when explicitly overriding');
    var index = fdlib.trie_get(config._varNamesTrie, varName);
    fdlib.ASSERT(index >= 0, 'should exist');
    fdlib.ASSERT_NORDOM(domain, true, fdlib.domain__debug);
  ...
  if (varName === true) {
    ...
    if (typeof varName !== 'string') fdlib.THROW('Var names should be a string or anonymous, was: ' + JSON.stringify(varName));
    if (!varName) fdlib.THROW('Var name cannot be empty string');
    if (String(parseInt(varName, 10)) === varName) fdlib.THROW("Don't use numbers as var names (" + varName + ')');
  ...
  if (varIndex < 100) {
    if (fdlib.trie_has(config._varNamesTrie, varName)) fdlib.THROW('Var name already part of this config. Probably a bug?', varName);
  }

  var solvedTo = fdlib.domain_getValue(domain);
  if (solvedTo !== fdlib.NOT_FOUND && !config.constantCache[solvedTo]) config.constantCache[solvedTo] = varIndex;
  fdlib.ASSERT_NORDOM(domain, true, fdlib.domain__debug);
  config.initialDomains[varIndex] = domain;
  config.allVarNames.push(varName);
  fdlib.trie_add(config._varNamesTrie, varName, varIndex);
  return varIndex;
}
/**
...
function config_setOption(config, optionName, optionValue, optionTarget) {
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(typeof optionName === 'string', 'option name is a string');
  fdlib.ASSERT(optionValue !== undefined, 'should get a value');
  fdlib.ASSERT(optionTarget === undefined || typeof optionTarget === 'string', 'the optional name is a string');

  if (optionName === 'varStratOverride') {
    fdlib.THROW('deprecated, should be wiped internally');
  }
  ...
  switch (optionName) {
    ...
    case 'varStrategy':
      {
        if (typeof optionValue === 'function') fdlib.THROW('functions no longer supported', optionValue);
        if (typeof optionValue === 'string') fdlib.THROW('strings should be passed on as {type:value}', optionValue);
        if (typeof optionValue !== 'object') fdlib.THROW('varStrategy should be object', optionValue);
        if (optionValue.name) fdlib.THROW('name should be type');
        if (optionValue.dist_name) fdlib.THROW('dist_name should be type');
        fdlib.ASSERT(!optionTarget, 'optionTarget is not used for varStrategy (this is not "per-var strat")');
        var vsc = config_createVarStratConfig(optionValue);

        if (fallback) {
          var rvsc = config.varStratConfig;
          fdlib.ASSERT(rvsc, 'there must be an existing config to add a fallback');
    ...
    case 'targeted_var_names':
      if (!optionValue || optionValue.length === 0) {
        fdlib.THROW('ONLY_USE_WITH_SOME_TARGET_VARS'); // Omit otherwise to target all
    ...
    case 'varValueStrat':
      ...
      fdlib.ASSERT(typeof optionTarget === 'string', 'expecting a name');
      if (!config.varDistOptions) config.varDistOptions = {};
      fdlib.ASSERT(!config.varDistOptions[optionTarget], 'should not be known yet');
      ...
      if (optionValue.valtype === 'markov') {
        ...
        if (!matrix) {
          if (optionValue.expandVectorsWith) {
            ...
            fdlib.THROW('FDO: markov var missing distribution (needs matrix or expandVectorsWith)');
          }
        }
        ...
        for (var _iterator = matrix, _isArray = Array.isArray(_iterator), _i3 = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
          ...
          var row = _ref;
          if (row.boolean) fdlib.THROW('row.boolean was deprecated in favor of row.boolVarName');
          if (row.booleanId !== undefined) fdlib.THROW('row.booleanId is no longer used, please use row.boolVarName');
          ...
          if (boolFuncOrName) {
            if (typeof boolFuncOrName !== 'string') {
              fdlib.THROW('row.boolVarName, if it exists, should be the name of a var or a func that returns that name, was/got: ' + boolFuncOrName + ' (' + typeof boolFuncOrName + ')');
            } // Store the var index


            row._boolVarIndex = fdlib.trie_get(config._varNamesTrie, boolFuncOrName);
          }
        }
      }
    ...
    case 'var':
      return fdlib.THROW('REMOVED. Replace `var` with `varStrategy`');

    case 'val':
      return fdlib.THROW('REMOVED. Replace `var` with `valueStrategy`');
    ...
    case 'rng':
      ...
      if (typeof optionValue === 'string') {
        ...
        fdlib.ASSERT(typeof optionValue === 'function', 'rng should be a preferably a string and otherwise a function');
...
function config_addPropagator(config, propagator) {
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(propagator._class === '$propagator', 'EXPECTING_PROPAGATOR');
...
function _config_addVarConditionally(varIndex, initialDomains, hash, propagatorIndex) {
  ...
  fdlib.ASSERT(typeof varIndex === 'number', 'must be number'); // Dont bother adding props on unsolved vars because they can't affect
...
function config_addConstraint(config, name, varNames, param) {
  ...
  fdlib.ASSERT(config && config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(varNames.every(function (e) {
  ...
  /* eslint no-fallthrough: "off" */
    ...
    case 'ring-div':
    case 'mul':
      fdlib.ASSERT(varNames.length === 3, 'MISSING_RESULT_VAR');
    ...
    case 'product':
      {
        ...
        if (resultVarName === undefined) {
          ...
          resultVarIndex = fdlib.trie_get(config._varNamesTrie, resultVarName);
          if (resultVarIndex < 0) fdlib.THROW('Vars must be defined before using them (' + resultVarName + ')');
        } else {
          fdlib.THROW("expecting result var name to be absent or a number or string: `" + resultVarName + "`");
        }
    ...
    default:
      fdlib.THROW("UNKNOWN_PROPAGATOR " + name);
...
function config_varNamesToIndexes(config, varNames) {
  ...
  for (var i = 0, n = varNames.length; i < n; ++i) {
    ...
    fdlib.ASSERT(typeof varName === 'string', 'var names should be strings here', varName, i, varNames);
    var varIndex = fdlib.trie_get(config._varNamesTrie, varName);
    if (varIndex === fdlib.TRIE_KEY_NOT_FOUND) fdlib.THROW('CONSTRAINT_VARS_SHOULD_BE_DECLARED', 'name=', varName, 'index=', i, 'names=', varNames);
...
function config_dedupeConstraint(config, constraintUI, resultVarName) {
  ...
  if (haveConstraint !== undefined) {
    fdlib.ASSERT(typeof haveConstraint === 'string', 'if not true or undefined, it should be a string');
    fdlib.ASSERT(resultVarName && typeof resultVarName === 'string', 'if it was recorded as a constraint-with-value then it should have a result var now as well'); // The constraint exists and had a result. map that result to this result for equivalent results.
...
function config_generatePropagators(config) {
  ...
  for (var _iterator2 = allConstraints, _isArray2 = Array.isArray(_iterator2), _i4 = 0, _iterator2 = _isArray2 ? _iterator2 : _iterator2[Symbol.iterator]();;) {
    ...
    if (constraint.varNames) {
      fdlib.getTerm().warn('saw constraint.varNames, converting to varIndexes, log out result and update test accordingly');
      constraint.varIndexes = constraint.varNames.map(function (name) {
        return fdlib.trie_get(config._varNamesTrie, name);
      });
    ...
    if (constraint.varIndexes[1] === -1) throw new Error('nope? ' + fdlib.INSPECT(constraint));
...
function config_generatePropagator(config, name, varIndexes, param, _constraint) {
  fdlib.ASSERT(config && config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(typeof name === 'string', 'NAME_SHOULD_BE_STRING');
  fdlib.ASSERT(Array.isArray(varIndexes), 'INDEXES_SHOULD_BE_ARRAY', JSON.stringify(_constraint));
  ...
  switch (name) {
    ...
    default:
      fdlib.THROW('UNEXPECTED_NAME: ' + name);
  }
}
...
function config_generateMarkovs(config) {
  ...
  for (var _i5 = 0, _Object$keys = Object.keys(varDistOptions); _i5 < _Object$keys.length; _i5++) {
    ...
    var varIndex = fdlib.trie_get(config._varNamesTrie, varName);
    if (varIndex < 0) fdlib.THROW('Found markov var options for an unknown var name (name=' + varName + ')');
  ...
  while (vsc) {
    if (vsc.priorityByName) {
      ...
      for (var i = 0, len = list.length; i < len; ++i) {
        var varIndex = fdlib.trie_get(config._varNamesTrie, list[i]);
        fdlib.ASSERT(varIndex !== fdlib.TRIE_KEY_NOT_FOUND, 'VARS_IN_PRIO_LIST_SHOULD_BE_KNOWN_NOW');
...
function config_init(config) {
  ...
  if (!config._varNamesTrie) {
    config._varNamesTrie = fdlib.trie_create(config.allVarNames);
  ...
  fdlib.ASSERT_VARDOMS_SLOW(config.initialDomains, fdlib.domain__debug);
  ...
  fdlib.ASSERT_VARDOMS_SLOW(config.initialDomains, fdlib.domain__debug);
  fdlib.ASSERT(config._varToPropagators, 'should have generated hash');
}
...
function exporter(config, vardoms, usePropagators, minimal, withDomainComments, realName) {
  ...
  var vars = varNames.map(function (varName, varIndex) {
    ...
    if (overrides && (overrides.valtype !== 'list' || overrides.list && overrides.list.length > 0)) {
      ...
      switch (overrides.valtype) {
        ...
        default:
          fdlib.getTerm().warn('Unknown value strategy override: ' + overrides.valtype);
  ...
  var constraints = usePropagators ? [] : config.allConstraints.map(function (constraint) {
    ...
    indexes.forEach(function (varIndex, i) {
      var v = fdlib.domain_getValue(domains[varIndex]);
      if (v >= 0) aliases[i] = v;
    ...
    if (typeof constraint.param === 'number') {
      var paramV = fdlib.domain_getValue(domains[constraint.param]);
    ...
    switch (constraint.name) {
      ...
      default:
        fdlib.getTerm().warn('unknown constraint: ' + constraint.name);
  ...
  var propagators = usePropagators ? config._propagators.map(function (propagator) {
    ...
    var v1 = varIndex1 >= 0 ? fdlib.domain_getValue(domains[varIndex1]) : -1;
    var name1 = v1 >= 0 ? v1 : varIndex1 < 0 ? undefined : indexToString(varIndex1);
    var v2 = varIndex2 >= 0 ? fdlib.domain_getValue(domains[varIndex2]) : -1;
    var name2 = v2 >= 0 ? v2 : varIndex2 < 0 ? undefined : indexToString(varIndex2);
    var v3 = varIndex3 >= 0 ? fdlib.domain_getValue(domains[varIndex3]) : -1;
    ...
    switch (propagator.name) {
      ...
      default:
        fdlib.getTerm().warn('unknown propagator: ' + propagator.name);
  ...
  '@custom val-strat = ' + config.valueStratName, vars.join('\n') || '# no vars', constraints.join('\n') || propagators.join('\n') || '# no constraints', '@custom targets ' + (config.targetedVars === 'all' ? ' = all' : '(' + config.targetedVars.map(function (varName) {
    return indexToString(fdlib.trie_get(config._varNamesTrie, varName));
...
function exporter_domstr(domain) {
  ...
  var arrdom = fdlib.domain_toArr(domain);
...
function importer(str, solver, _debug) {
  ...
  function parseAll() {
    ...
    var r = solver.product(refs, solver.decl(undefined, [1, fdlib.SUP]));
    skipWhitespaces();
    is(')', 'ALL closer');
    return r;
  }
  ...
  function parseSome() {
    ...
    skipWhitespaces();
    var refs = parseVexpList();
    solver.sum(refs, solver.decl(undefined, [1, fdlib.SUP]));
    skipWhitespaces();
    is(')', 'some closer');
    expectEol();
  }
  ...
  function THROW(msg) {
    if (_debug) {
      fdlib.getTerm().log(str.slice(0, pointer) + '##|PARSER_IS_HERE[' + msg + ']|##' + str.slice(pointer));
    }
...
function space_toConfig(space, config) {
  ...
  for (var i = 0, n = allVarNames.length; i < n; i++) {
    ...
    newDomains[i] = fdlib.domain_toStr(domain);
  }
...
function space_createNew(vardoms, unsolvedVarIndexes, _depth, _child, _path) {
  fdlib.ASSERT(typeof vardoms === 'object' && vardoms, 'vars should be an object', vardoms);
  var space = {
    _class: '$space',
    vardoms: vardoms,
...
function space_initFromConfig(space, config) {
  fdlib.ASSERT(space._class === '$space', 'EXPECTING_SPACE');
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
...
function space_initializeUnsolvedVars(space, config) {
  ...
  if (targetVarNames === 'all') {
    for (var varIndex = 0, n = vardoms.length; varIndex < n; ++varIndex) {
      if (!fdlib.domain_isSolved(vardoms[varIndex])) {
    ...
    fdlib.ASSERT(Array.isArray(targetVarNames), 'expecting targetVarNames to be an array or the string `all`', targetVarNames);
    fdlib.ASSERT(targetVarNames.every(function (e) {
...
function space_addVarNameToUnsolved(varName, varNamesTrie, vardoms, unsolvedVarIndexes) {
  var varIndex = fdlib.trie_get(varNamesTrie, varName);
  if (varIndex === fdlib.TRIE_KEY_NOT_FOUND) fdlib.THROW('E_VARS_SHOULD_EXIST_NOW [' + varName + ']');
...
function space_propagate(space, config) {
  ...
  var cells = Math.ceil(space._unsolved.length * fdlib.TRIE_NODE_SIZE * 0.3);
  var changedTrie = fdlib.trie_create(fdlib.TRIE_EMPTY, cells); // Track changed vars per cycle in this epoch

  var cycles = 0;
  fdlib.ASSERT(typeof cycles === 'number', 'cycles is a number?');
  fdlib.ASSERT(changedTrie._class === '$trie', 'trie is a trie?');
...
function space_propagateStep(space, config, propagator, changedVars, changedTrie, cycleIndex) {
  ...
  fdlib.ASSERT(index1 !== 'undefined', 'all props at least use the first var...');
  var domain1 = vardoms[index1];
  var domain2 = index2 !== undefined && vardoms[index2];
  var domain3 = index3 !== undefined && vardoms[index3];
  fdlib.ASSERT_NORDOM(domain1, true, fdlib.domain__debug);
  fdlib.ASSERT(domain2 === undefined || fdlib.ASSERT_NORDOM(domain2, true, fdlib.domain__debug));
  fdlib.ASSERT(domain3 === undefined || fdlib.ASSERT_NORDOM(domain3, true, fdlib.domain__debug));
  fdlib.ASSERT(typeof stepper === 'function', 'stepper should be a func'); // TODO: if we can get a "solved" state here we can prevent an isSolved check later...

  stepper(space, config, index1, index2, index3, arg1, arg2, arg3, arg4, arg5, arg6);

  if (domain1 !== vardoms[index1]) {
    if (fdlib.domain_isEmpty(vardoms[index1])) {
      return true; // Fail
    }
  ...
  if (index2 !== undefined && domain2 !== vardoms[index2]) {
    if (fdlib.domain_isEmpty(vardoms[index2])) {
      return true; // Fail
    }
  ...
  if (index3 !== undefined && index3 !== -1 && domain3 !== vardoms[index3]) {
    if (fdlib.domain_isEmpty(vardoms[index3])) {
      return true; // Fail
    }
...
function space_recordChange(varIndex, changedTrie, changedVars, cycleIndex) {
  if (typeof varIndex === 'number') {
    var status = fdlib.trie_getNum(changedTrie, varIndex);

    if (status !== cycleIndex) {
      changedVars.push(varIndex);
      fdlib.trie_addNum(changedTrie, varIndex, cycleIndex);
    }
  } else {
    fdlib.ASSERT(Array.isArray(varIndex), 'index1 is always used');
...
function space_getVarSolveState(space, varIndex) {
  fdlib.ASSERT(typeof varIndex === 'number', 'VAR_SHOULD_BE_INDEX');
  var domain = space.vardoms[varIndex];

  if (fdlib.domain_isEmpty(domain)) {
    return false;
  }

  var value = fdlib.domain_getValue(domain);
  if (value !== fdlib.NO_SUCH_VALUE) return value;
  return fdlib.domain_toArr(domain);
}
/**
...
function space_generateVars(space, config) {
  ...
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
  var vardoms = space.vardoms;
  fdlib.ASSERT(vardoms, 'expecting var domains');
  var initialDomains = config.initialDomains;
  fdlib.ASSERT(initialDomains, 'config should have initial vars');
  var allVarNames = config.allVarNames;
  fdlib.ASSERT(allVarNames, 'config should have a list of vars');

  for (var varIndex = 0, len = allVarNames.length; varIndex < len; varIndex++) {
    var domain = initialDomains[varIndex];
    fdlib.ASSERT_NORDOM(domain, true, fdlib.domain__debug);
    vardoms[varIndex] = fdlib.domain_toSmallest(domain);
  }
}
...
function distribution_getFunc(distName) {
  switch (distName) {
    ...
    case 'throw':
      return fdlib.THROW('Throwing an error because var-strat requests it');

    default:
      return fdlib.THROW('unknown next var func', distName);
  }
}
/**
...
function _distribution_varFindBest(space, config, fitnessFunc, varStratConfig) {
  var i = 0;
  var unsolvedVarIndexes = space._unsolved;
  fdlib.ASSERT(unsolvedVarIndexes.length, 'there should be unsolved vars left to pick (caller should ensure this)');
  ...
  if (fitnessFunc) {
    for (var len = unsolvedVarIndexes.length; i < len; i++) {
      ...
      fdlib.ASSERT(typeof varIndex === 'number', 'VAR_INDEX_SHOULD_BE_NUMBER');
      fdlib.ASSERT(space.vardoms[varIndex] !== undefined, 'expecting each varIndex to have an domain', varIndex);
  ...
  fdlib.ASSERT(typeof bestVarIndex === 'number', 'VAR_INDEX_SHOULD_BE_NUMBER');
  fdlib.ASSERT(bestVarIndex >= 0, 'VAR_INDEX_SHOULD_BE_POSITIVE');
  return bestVarIndex;
} // #####
...
function distribution_varByMinSize(space, config, varIndex1, varIndex2) {
  fdlib.ASSERT(space._class === '$space', 'SPACE_SHOULD_BE_SPACE');
  fdlib.ASSERT(typeof varIndex1 === 'number', 'INDEX_SHOULD_BE_NUMBER');
  fdlib.ASSERT(typeof varIndex2 === 'number', 'INDEX_SHOULD_BE_NUMBER');
  var n = fdlib.domain_size(space.vardoms[varIndex1]) - fdlib.domain_size(space.vardoms[varIndex2]);
...
function distribution_varByMin(space, config, varIndex1, varIndex2) {
  ...
  fdlib.ASSERT(typeof varIndex1 === 'number', 'INDEX_SHOULD_BE_NUMBER');
  fdlib.ASSERT(typeof varIndex2 === 'number', 'INDEX_SHOULD_BE_NUMBER');
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex1]);
  fdlib.ASSERT_NORDOM(space.vardoms[varIndex2]);
  fdlib.ASSERT(space.vardoms[varIndex1] && space.vardoms[varIndex2], 'EXPECTING_NON_EMPTY');
  var n = fdlib.domain_min(space.vardoms[varIndex1]) - fdlib.domain_min(space.vardoms[varIndex2]);
  if (n < 0) return BETTER;
  if (n > 0) return WORSE;
  return SAME;
}
...
function distribution_varByMax(space, config, varIndex1, varIndex2) {
  ...
  fdlib.ASSERT(typeof varIndex1 === 'number', 'INDEX_SHOULD_BE_NUMBER');
  fdlib.ASSERT(typeof varIndex2 === 'number', 'INDEX_SHOULD_BE_NUMBER');
  var n = fdlib.domain_max(space.vardoms[varIndex1]) - fdlib.domain_max(space.vardoms[varIndex2]);
  if (n > 0) return BETTER;
  if (n < 0) return WORSE;
  return SAME;
}
...
function distribution_varByMarkov(space, config, varIndex1, varIndex2, varStratConfig) {
  ...
  fdlib.ASSERT(typeof varName1 === 'string', 'VAR_NAME_SHOULD_BE_STRING');
  ...
  fdlib.ASSERT(typeof varName2 === 'string', 'VAR_NAME_SHOULD_BE_STRING');
...
function distribution_varByList(space, config, varIndex1, varIndex2, varStratConfig) {
  fdlib.ASSERT(space._class === '$space', 'SPACE_SHOULD_BE_SPACE');
  fdlib.ASSERT(typeof varIndex1 === 'number', 'INDEX_SHOULD_BE_NUMBER');
  fdlib.ASSERT(typeof varIndex2 === 'number', 'INDEX_SHOULD_BE_NUMBER'); // Note: config._priorityByIndex is compiled by FDO#prepare from given priorityByName
  ...
  fdlib.ASSERT(p1 !== 0, 'SHOULD_NOT_USE_INDEX_ZERO');
  fdlib.ASSERT(p2 !== 0, 'SHOULD_NOT_USE_INDEX_ZERO');
  ...
  fdlib.ASSERT(p1 < p2, 'A_CANNOT_GET_SAME_INDEX_FOR_DIFFERENT_NAME');
  if (inverted) return BETTER;
  return WORSE;
}
...
function distribution_varFallback(space, config, varIndex1, varIndex2, fallbackConfig) {
  ...
  switch (distName) {
    ...
    case 'throw':
      return fdlib.THROW('nope');

    default:
      return fdlib.THROW("Unknown var dist fallback name: " + distName);
  }
}
...
function distribution_markovSampleNextFromDomain(domain, probVector, valLegend, randomFunc, rngIsNormalized) {
  ...
  fdlib.ASSERT(Boolean(valLegend), 'A_SHOULD_HAVE_VAL_LEGEND');
  fdlib.ASSERT(probVector.length <= valLegend.length, 'A_PROB_VECTOR_SIZE_SHOULD_BE_LTE_LEGEND'); // Make vector & legend for available values only
  ...
  for (var index = 0; index < probVector.length; index++) {
    ...
    if (prob > 0) {
      ...
      if (fdlib.domain_containsValue(domain, value)) {
        totalProb += prob;
...
function _distribution_markovRoll(rng, totalProb, cumulativeProbVector, valueLegend, rngIsNormalized) {
  ...
  if (rngIsNormalized) {
    ...
    fdlib.ASSERT(rngRoll >= 0, 'RNG_SHOULD_BE_NORMALIZED');
    fdlib.ASSERT(rngRoll < 1, 'RNG_SHOULD_BE_NORMALIZED');
...
function distribute_getNextDomainForVar(space, config, varIndex, choiceIndex) {
  ...
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(typeof varIndex === 'number', 'VAR_INDEX_SHOULD_BE_NUMBER');
  fdlib.ASSERT(space.vardoms[varIndex] && !fdlib.domain_isSolved(space.vardoms[varIndex]), 'CALLSITE_SHOULD_PREVENT_DETERMINED'); // TODO: test
  ...
  fdlib.ASSERT(typeof varName === 'string', 'VAR_NAME_SHOULD_BE_STRING');
  ...
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_CHOICE, function (log) {
    return log('distribute_getNextDomainForVar; index', varIndex, 'is now', fdlib.domain__debug(domain));
  });
  return domain;
}
...
function _distribute_getNextDomainForVar(stratName, space, config, varIndex, choiceIndex) {
  ...
  switch (stratName) {
    ...
    case 'throw':
      return fdlib.THROW('Throwing an error because val-strat requests it');

    default:
      fdlib.THROW('unknown next var func', stratName);
  }
}
/**
...
function distribution_valueByList(space, config, varIndex, choiceIndex) {
  ...
  fdlib.ASSERT(config._class === '$config', 'EXPECTING_CONFIG');
  fdlib.ASSERT(typeof varIndex === 'number', 'VAR_INDEX_SHOULD_BE_NUMBER');
  fdlib.ASSERT(typeof choiceIndex === 'number', 'CHOICE_SHOULD_BE_NUMBER');
  var domain = space.vardoms[varIndex];
  var varName = config.allVarNames[varIndex];
  fdlib.ASSERT(typeof varName === 'string', 'VAR_NAME_SHOULD_BE_STRING');
  fdlib.ASSERT(domain && !fdlib.domain_isSolved(domain), 'DOMAIN_SHOULD_BE_UNDETERMINED');
  var configVarDistOptions = config.varDistOptions;
  fdlib.ASSERT(configVarDistOptions, 'space should have config.varDistOptions');
  fdlib.ASSERT(configVarDistOptions[varName], 'there should be distribution options available for every var', varName);
  fdlib.ASSERT(configVarDistOptions[varName].list, 'there should be a distribution list available for every var', varName);
  ...
  if (varDistOptions.fallback) {
    ...
    fdlib.ASSERT(fallbackName, 'should have a fallback type');
    fdlib.ASSERT(fallbackName !== 'list', 'prevent recursion loops');
  }
  ...
  switch (choiceIndex) {
    case FIRST_CHOICE:
      {
        var nextValue = fdlib.domain_getFirstIntersectingValue(domain, list);

        if (nextValue === fdlib.NO_SUCH_VALUE) {
        ...
        return fdlib.domain_createValue(nextValue);
      }

    case SECOND_CHOICE:
      if (space._lastChosenValue >= 0) {
        return fdlib.domain_removeValue(domain, space._lastChosenValue);
      }

      return _distribute_getNextDomainForVar(fallbackName || 'naive', space, config, varIndex, SECOND_CHOICE);

    default:
      fdlib.ASSERT(choiceIndex === THIRD_CHOICE, 'SHOULD_NOT_CALL_MORE_THAN_THRICE');
      return NO_CHOICE;
  }
}
/**
    ...
    case FIRST_CHOICE:
      {
        ...
        return fdlib.domain_createValue(minValue);
      }

    case SECOND_CHOICE:
      // Cannot lead to empty domain because lo can only be SUP if
      // domain was solved and we assert it wasn't.
      fdlib.ASSERT(space._lastChosenValue >= 0, 'first choice should set this property and it should at least be 0', space._lastChosenValue);
      return fdlib.domain_removeValue(domain, space._lastChosenValue);
    ...
    case FIRST_CHOICE:
      {
        ...
        return fdlib.domain_createValue(maxValue);
      }

    case SECOND_CHOICE:
      // Cannot lead to empty domain because hi can only be SUB if
      // domain was solved and we assert it wasn't.
      fdlib.ASSERT(space._lastChosenValue > 0, 'first choice should set this property and it should at least be 1', space._lastChosenValue);
      return fdlib.domain_removeValue(domain, space._lastChosenValue);
...
function distribution_valueByMid(space, varIndex, choiceIndex) {
  ...
  switch (choiceIndex) {
    case FIRST_CHOICE:
      {
        var middle = fdlib.domain_middleElement(domain);
        space._lastChosenValue = middle;
        return fdlib.domain_createValue(middle);
      }
...
function distribution_valueBySplitMin(space, varIndex, choiceIndex) {
  ...
  fdlib.ASSERT_NORDOM(domain);
  fdlib.ASSERT(domain && !fdlib.domain_isSolved(domain), 'DOMAIN_SHOULD_BE_UNDETERMINED');
  var max = fdlib.domain_max(domain);

  switch (choiceIndex) {
    case FIRST_CHOICE:
      {
        // TOFIX: can do this more optimal if coding it out explicitly
        var min = fdlib.domain_min(domain);
        ...
        return fdlib.domain_intersection(domain, fdlib.domain_createRange(min, mmhalf));
      }

    case SECOND_CHOICE:
      {
        fdlib.ASSERT(space._lastChosenValue >= 0, 'first choice should set this property and it should at least be 0', space._lastChosenValue); // Note: domain is not determined so the operation cannot fail
        // Note: this must do some form of intersect, though maybe not constrain

        return fdlib.domain_intersection(domain, fdlib.domain_createRange(space._lastChosenValue + 1, max));
      }
    ...
    case FIRST_CHOICE:
      {
        ...
        return fdlib.domain_intersection(domain, fdlib.domain_createRange(mmhalf + 1, max));
      }
    ...
    case SECOND_CHOICE:
      {
        ...
        return fdlib.domain_intersection(domain, fdlib.domain_createRange(min, space._lastChosenValue));
      }
    ...
    case FIRST_CHOICE:
      {
        ...
        fdlib.ASSERT(distOptions, 'markov vars should have  distribution options');
        var expandVectorsWith = distOptions.expandVectorsWith;
        fdlib.ASSERT(distOptions.matrix, 'there should be a matrix available for every var');
        fdlib.ASSERT(distOptions.legend || typeof expandVectorsWith === 'number' && expandVectorsWith >= 0, 'every var should have a legend or expandVectorsWith set');
        var random = distOptions.random || config._defaultRng;
        fdlib.ASSERT(typeof random === 'function', 'RNG_SHOULD_BE_FUNCTION'); // Note: expandVectorsWith can be 0, so check with null
        ...
        fdlib.ASSERT(fdlib.domain_containsValue(domain, value), 'markov picks a value from the existing domain so no need for a constrain below');
        space._lastChosenValue = value;
        return fdlib.domain_createValue(value);
      }

    case SECOND_CHOICE:
      {
        var lastValue = space._lastChosenValue;
        fdlib.ASSERT(typeof lastValue === 'number', 'should have cached previous value');
        var newDomain = fdlib.domain_removeValue(domain, lastValue);
        fdlib.ASSERT(domain, 'domain cannot be empty because only one value was removed and the domain is asserted to be not solved above');
        fdlib.ASSERT_NORDOM(newDomain, true, fdlib.domain__debug);
        return newDomain;
      }
...
function search_depthFirst(state, config, dbgCallback) {
  ...
  while (stack.length > 0 && !config.aborted) {
    fdlib.ASSERT_LOG(fdlib.LOG_FLAG_SEARCH, function (log) {
      return log('');
    });
    fdlib.ASSERT_LOG(fdlib.LOG_FLAG_SEARCH, function (log) {
      return log('');
...
function search_depthFirstLoop(space, config, stack, state) {
  ...
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_SEARCH, function (log) {
    return log('  -', Math.min(10, space.vardoms.length) + '/' + space.vardoms.length, 'domains:', space.vardoms.slice(0, 10).map(fdlib.domain__debug).join(', '));
  });
  fdlib.ASSERT_LOG(fdlib.LOG_FLAG_SEARCH, function (log) {
...
function search_createNextSpace(space, config) {
  ...
  fdlib.ASSERT(typeof varIndex === 'number', 'VAR_INDEX_SHOULD_BE_NUMBER');
  fdlib.ASSERT(varIndex >= 0, 'VAR_INDEX_SHOULD_BE_POSITIVE');

  if (varIndex !== fdlib.NO_SUCH_VALUE) {
    var domain = space.vardoms[varIndex];
...
function () {
  ...
  function FDO(options) {
    ...
    this._class = 'fdo';
    if (options.logger) fdlib.setTerm(options.logger);
    this.logging = options.log || fdlib.LOG_NONE;
    ...
    fdlib.ASSERT(options._class !== '$config', 'config should be passed on in a config property of options');
    ...
    if (options.config) {
      ...
      if (config.initialDomains) {
        ...
        for (var i = 0, len = initialDomains.length; i < len; ++i) {
          ...
          if (domain.length === 0) domain = fdlib.domain_createEmpty();
          initialDomains[i] = fdlib.domain_anyToSmallest(domain);
        }
      }
  ...
  _proto.num = function num(_num) {
    if (typeof _num !== 'number') {
      fdlib.THROW("FDO#num: expecting a number, got " + _num + " (a " + typeof _num + ")");
    }

    if (isNaN(_num)) {
      fdlib.THROW('FDO#num: expecting a number, got NaN');
    }
  ...
  _proto.decl = function decl(varName, domainOrValue, distributionOptions, _allowEmpty, _override) {
    if (varName === '') fdlib.THROW('Var name can not be the empty string');
    fdlib.ASSERT(varName === undefined || typeof varName === 'string', 'var name should be undefined or a string');
    fdlib.ASSERT(distributionOptions === undefined || typeof distributionOptions === 'object', 'options must be omitted or an object');
    var arrdom = typeof domainOrValue === 'number' ? [domainOrValue, domainOrValue] : domainOrValue || [fdlib.SUB, fdlib.SUP];
    fdlib.ASSERT_ARRDOM(arrdom);
    if (arrdom.length === 0 && !_allowEmpty) fdlib.THROW('EMPTY_DOMAIN_NOT_ALLOWED');
    ...
    if (distributionOptions) {
      if (distributionOptions.distribute) fdlib.THROW('Use `valtype` to set the value distribution strategy');
  ...
  _proto.declRange = function declRange(varName, lo, hi, options) {
    fdlib.ASSERT(typeof lo === 'number', 'LO_SHOULD_BE_NUMBER');
    fdlib.ASSERT(typeof hi === 'number', 'HI_SHOULD_BE_NUMBER');
    fdlib.ASSERT(typeof options === 'object' || options === undefined, 'EXPECTING_OPTIONS_OR_NOTHING');
  ...
  _proto.gte = function gte(A, B) {
    fdlib.ASSERT(!Array.isArray(A), 'NOT_ACCEPTING_ARRAYS');
    ...
    if (options._tostring || options._debug || options._debugConfig || options._debugSpace || options._debugSolver) {
      dbgCallback = function dbgCallback(epoch) {
        if ((options._debugDelay | 0) >= epoch) {
          if (options._tostring) fdlib.getTerm().log(exporter(_this.config));
          if (options._debug) _this._debugLegible();
          if (options._debugConfig) _this._debugConfig();

          if (process.env.NODE_ENV !== 'production') {
            if (options._debugSpace) fdlib.getTerm().log('## _debugSpace:\n', fdlib.INSPECT(_this._space));
          }
  ...
  _proto._prepare = function _prepare(options, log) {
    ...
    fdlib.ASSERT(log === undefined || log >= fdlib.LOG_MIN && log <= fdlib.LOG_MAX, 'log level should be a valid value or be undefined (in tests)');

    if (log >= fdlib.LOG_STATS) {
      fdlib.getTerm().log('      - FD Preparing...');

      fdlib.getTerm().time('      - FD Prepare Time');
    }
    ...
    this.state.stack = [];
    this._prepared = true;
    if (log >= fdlib.LOG_STATS) fdlib.getTerm().timeEnd('      - FD Prepare Time');
  }
  /**
  ...
  _proto._run = function _run(max, log, dbgCallback) {
    fdlib.ASSERT(typeof max === 'number', 'max should be a number', max);
    fdlib.ASSERT(log >= fdlib.LOG_MIN && log <= fdlib.LOG_MAX, 'log level should be a valid value');
    fdlib.ASSERT(this._prepared, 'must run #prepare before #run');
    this._prepared = false;
    ...
    for (var i = 0, n = vardoms.length; i < n; ++i) {
      if (fdlib.domain_isEmpty(vardoms[i])) {
  ...
  _proto.hasVar = function hasVar(varName) {
    return fdlib.trie_get(this.config._varNamesTrie, varName) >= 0;
  }
  /**
  ...
  _proto.setValueDistributionFor = function setValueDistributionFor(varName, options) {
    fdlib.ASSERT(typeof varName === 'string', 'var name should be a string', varName);
    fdlib.ASSERT(typeof options === 'object', 'value strat options should be an object');
  ...
  _proto._debugLegible = function _debugLegible() {
    ...
    term.log('\n## _debug:\n');
    term.log('- config:');
    term.log(fdlib.INSPECT(clone));
    term.log('- vars (' + names.length + '):');
    term.log(names.map(function (name, index) {
      return (index) + ": " + fdlib.domain__debug(domains[index]) + " " + (name === String(index) ? '' : ' // ' + name);
    }).join('\n'));
    ...
    term.log(constraints.map(function (c, index) {
      if (c.param === undefined) {
        return (index) + ": " + c.name + "(" + c.varIndexes + ")      --->  " + c.varIndexes.map(function (index) {
          return fdlib.domain__debug(domains[index]);
        }).join(',  ');
      }

      if (c.name === 'reifier') {
        return (index) + ": " + c.name + "[" + c.param + "](" + c.varIndexes + ")      --->  " + fdlib.domain__debug(domains[c.varIndexes[0]]) + " " + c.param + " " + fdlib.domain__debug(domains[c.varIndexes[1]]) + " = " + fdlib.domain__debug(domains[c.varIndexes[2]]);
      }

      return (index) + ": " + c.name + "(" + c.varIndexes + ") = " + c.param + "      --->  " + c.varIndexes.map(function (index) {
        return fdlib.domain__debug(domains[index]);
      }).join(',  ') + " -> " + fdlib.domain__debug(domains[c.param]);
    }).join('\n'));
    term.log('##/\n');
  };
  ...
  _proto._debugSolver = function _debugSolver() {
    ...
    for (var varIndex = 0; varIndex < names.length; ++varIndex) {
      term.log('  ', varIndex, ':', names[varIndex], ':', fdlib.domain__debug(config.initialDomains[varIndex]));
    }
    ...
    for (var _i3 = 0; _i3 < propagators.length; ++_i3) {
      term.log('  ', _i3, ':', propagators[_i3].name + (propagators[_i3].name === 'reified' ? '(' + propagators[_i3].arg3 + ')' : ''), ':', propagators[_i3].index1, propagators[_i3].index2, propagators[_i3].index3, '->', fdlib.domain__debug(config.initialDomains[propagators[_i3].index1]), fdlib.domain__debug(config.initialDomains[propagators[_i3].index2]), fdlib.domain__debug(config.initialDomains[propagators[_i3].index3]));
    }
  ...
  _proto._debugConfig = function _debugConfig() {
    ...
    config.initialDomains = config.initialDomains.map(fdlib.domain__debug);

    fdlib.getTerm().log('## _debugConfig:\n', fdlib.INSPECT(config));
  }
  /**
  ...
  _proto.imp = function imp(s, _debug) {
    ...
    // term.log('##y##');
    if (this.logging) {
      fdlib.getTerm().log('      - FD Importing DSL; ' + s.length + ' bytes');

      fdlib.getTerm().time('      - FD Import Time:');
    }

    var solver = importer(s, this, _debug);

    if (this.logging) {
      fdlib.getTerm().timeEnd('      - FD Import Time:');
    }
  ...
  FDO.domainFromList = function domainFromList(list) {
    return fdlib.domain_fromListToArrdom(list);
  }
  /**
  ...
  FDO.getTerm = function getTerm() {
    return fdlib.getTerm();
  }
  /**
  ...
  FDO.setTerm = function setTerm(term) {
    return fdlib.setTerm(term);
  };

  FDO.dsl = function dsl() {
    fdlib.THROW('FDO.dsl: use FDO.solve()');
  };

  FDO.imp = function imp() {
    fdlib.THROW('FDO.imp: use FDO.solve()');
  }
  /**
...
function _clone(value) {
  switch (typeof value) {
    ...
    default:
      fdlib.THROW('config value what?', value);
  }
}
/**
...
function solver_runLoop(state, config, max, dbgCallback) {
  ...
  while (state.more && list.length < max) {
    ...
    if (state.status !== 'end') {
      ...
      fdlib.ASSERT_LOG(fdlib.LOG_FLAG_SOLUTIONS, function (log) {
...
function solver_getSolutions(solvedSpaces, config, solutions, log) {
  fdlib.ASSERT(Array.isArray(solutions), 'solutions target object should be an array');

  if (log >= fdlib.LOG_STATS) {
    fdlib.getTerm().time('      - FD Solution Construction Time');
  }
  ...
  for (var i = 0; i < solvedSpaces.length; ++i) {
    ...
    if (log >= fdlib.LOG_SOLVES) {
      fdlib.getTerm().log('      - FD solution() ::::::::::::::::::::::::::::');

      fdlib.getTerm().log(JSON.stringify(solution));
  ...
  if (log >= fdlib.LOG_STATS) {
    fdlib.getTerm().timeEnd('      - FD Solution Construction Time');
  }
}

module.exports = FDO;
//# sourceMappingURL=fdo.js.map
