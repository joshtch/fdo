...
describe('fdo/propagators/eq.spec', () => {
  describe('propagator_eqStepBare', () => {
    ...
    test('should expect args', () => {
      ...
      expect(_ => {
        propagator_eqStepBare(
          ...
          config.allVarNames.indexOf('B')
        );
      }).not.toThrow();
      expect(_ => {
        propagator_eqStepBare(space, config);
      }).toThrow('VAR_INDEX_SHOULD_BE_NUMBER');
      expect(_ => {
        propagator_eqStepBare(space, config, config.allVarNames.indexOf('A'));
      }).toThrow('VAR_INDEX_SHOULD_BE_NUMBER');
      expect(_ => {
        propagator_eqStepBare(
          space,
    ...
    test('with array should split a domain if it covers multiple ranges of other domain', () => {
      const config = config_create();
    ...
    test('with number should split a domain if it covers multiple ranges of other domain', () => {
      const config = config_create();
      config_addVarRange(config, 'A', SUB, 15);
      config_addVarDomain(
        config,
        'B',
        fixt_arrdom_nums(0, 1, 2, 3, 4, 5, 10, 11, 12, 13, 14, 15)
      );
      const space = space_createRoot();
      space_initFromConfig(space, config);
      const A = config.allVarNames.indexOf('A');
      ...
      const C = fixt_dom_nums(0, 1, 2, 3, 4, 5, 10, 11, 12, 13, 14, 15);
    ...
    describe('when v1 != v2', () => {
      function testThis(left, right, result) {
        test(`should not change anything (left-right): ${[
          left,
          right,
          result,
        ].join('|')}`, () => {
      ...
      testThis(
        fixt_dom_nums(SUB, 1),
        fixt_arrdom_range(1, SUP),
        fixt_dom_solved(1)
      );
      testThis(
        fixt_arrdom_ranges([0, 10], [20, 30], [40, 50]),
        fixt_dom_nums(5, 5),
        fixt_dom_solved(5)
      );
      testThis(
        fixt_arrdom_ranges([0, 10], [20, 30], [40, 50]),
        fixt_arrdom_ranges([5, 15], [25, 35]),
        fixt_dom_nums(5, 6, 7, 8, 9, 10, 25, 26, 27, 28, 29, 30)
      );
      testThis(
        fixt_arrdom_ranges([0, 10], [20, 30], [40, 50]),
        fixt_arrdom_ranges([SUB, SUP]),
    ...
    describe('with LOG', () => {
      ...
      test('should improve test coverage by enabling logging', () => {
        ...
        config_addVarDomain(
          config,
          'B',
          fixt_arrdom_ranges([0, 10], [20, 300])
        );
        const space = space_createRoot();
        space_initFromConfig(space, config);
  ...
  describe('propagator_eqStepWouldReject', () => {
    ...
    test('regression', () => {
      expect(
        propagator_eqStepWouldReject(fixt_dom_range(1, 64), fixt_dom_nums(1))
      ).toBe(false);
      expect(
        propagator_eqStepWouldReject(fixt_dom_range(1, 64), fixt_dom_solved(1))
      ).toBe(false);
    });
  });
});
