...
describe('fdo/propagators/lt.spec', () => {
  ...
  test('should require two vars', () => {
    ...
    expect(() => {
      propagator_ltStepBare(space, 'A');
    }).toThrow('VAR_INDEX_SHOULD_BE_NUMBER');
    expect(() => {
      propagator_ltStepBare(space, undefined, 'B');
    }).toThrow('VAR_INDEX_SHOULD_BE_NUMBER');
  });
  ...
  describe('with array', () => {
    ...
    test('should be able to drop last range in v1', () => {
      ...
      propagator_ltStepBare(space, config, A, B);
      expect(space.vardoms[A]).toEqual(
  ...
  describe('with numbers', () => {
    test('should throw for empty domain', () => {
      ...
      expect(_ => {
        propagator_ltStepBare(space, config, A, D);
      }).toThrow('SHOULD_NOT_BE_REJECTED');
      expect(_ => {
        propagator_ltStepBare(space, config, C, B);
      }).toThrow('SHOULD_NOT_BE_REJECTED');
      expect(_ => {
        propagator_ltStepBare(space, config, C, D);
      }).toThrow('SHOULD_NOT_BE_REJECTED');
    });
