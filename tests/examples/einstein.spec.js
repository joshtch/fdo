...
describe('src/einstein.spec', () => {
  ...
  test('puzzle with api', () => {
    ...
    const blue = solver.decl('blue', 0);
    const green = solver.decl('green', 1);
    const red = solver.decl('red', 2);
    const white = solver.decl('white', 3);
    const yellow = solver.decl('yellow', 4);
    const C = [blue, green, red, white, yellow];

    const Dane = solver.decl('danish', 0);
    const Englishman = solver.decl('english', 1);
    const German = solver.decl('german', 2);
    const Swede = solver.decl('swedish', 3);
    const Norwegian = solver.decl('norwegian', 4);
    const N = [Dane, Englishman, German, Swede, Norwegian];

    const bier = solver.decl('bier', 0);
    const coffee = solver.decl('coffee', 1);
    const milk = solver.decl('milk', 2);
    const tea = solver.decl('tea', 3);
    const water = solver.decl('water', 4);
    const D = [bier, coffee, milk, tea, water];

    const Blend = solver.decl('blend', 0);
    const BlueMaster = solver.decl('bluemaster', 1);
    const Dunhill = solver.decl('dunhill', 2);
    const PallMall = solver.decl('pallmall', 3);
    const Prince = solver.decl('prince', 4);
    const S = [Blend, BlueMaster, Dunhill, PallMall, Prince];

    const birds = solver.decl('birds', 0);
    const cats = solver.decl('cats', 1);
    const dogs = solver.decl('dogs', 2);
    const fish = solver.decl('fish', 3);
    const horses = solver.decl('horses', 4);
    const P = [birds, cats, dogs, fish, horses];
    ...
    H.forEach(i =>
      ['C', 'N', 'S', 'D', 'P'].forEach(type => solver.decl(type + i, [0, 4]))
    );

    const Cn = ['C0', 'C1', 'C2', 'C3', 'C4'];
    const Nn = ['N0', 'N1', 'N2', 'N3', 'N4'];
    const Dn = ['D0', 'D1', 'D2', 'D3', 'D4'];
    const Sn = ['S0', 'S1', 'S2', 'S3', 'S4'];
    const Pn = ['P0', 'P1', 'P2', 'P3', 'P4'];
    solver.distinct(Cn);
    solver.distinct(Nn);
    solver.distinct(Dn);
    solver.distinct(Sn);
    ...
    H.forEach(i => {
      ...
      const ifThen = (a, b) => ifThenHouse(a, b, i);
      // "iif house[i]=x then house[i]=y"
      const ifThenHouse = (a, b, houseNumber) =>
        _iif(hash[a] + houseNumber, a, hash[b] + houseNumber, b);
      // "iif A=v1 then B=v2"
      const _iif = (ha, va, hb, vb) =>
        solver.eq(solver.isEq(ha, va), solver.isEq(hb, vb));
      // "iif house=x then house+=delta=y" (+1 or -1 for either neighbor)
      const ifThenNeighbor = (a, b, delta) =>
        _iif(hash[a] + i, a, hash[b] + (i + delta), b);
      // "iif house=x then either neighbor=y (not both)"
      const ifThenEitherNeighbor = (a, b) => {
      ...
      const ifThenXor = (a, n, b, c, m) =>
        solver.lte(
          solver.isEq(a, n),
          solver.isNeq(solver.isEq(b, m), solver.isEq(c, m))
        );
    ...
    solver.solve({
      ...
      log: 1,
      max: 2,
      vars: [].concat(Cn, Nn, Dn, Sn, Pn),
    });
    // Solution count
    expect(solver.solutions).toHaveLength(1); // Only has one solution
  ...
  test('puzzle with dsl', () => {
    ...
    const dsl = `
      ...
      : C0 [0 4] # house_color_of_first_house
      : C1 [0 4]
      : C2 [0 4]
      : C3 [0 4]
      : C4 [0 4]
      : N0 [0 4] # nationality_of_first_house
      : N1 [0 4]
      : N2 [0 4]
      : N3 [0 4]
      : N4 [0 4]
      : D0 [0 4] # drink_of_first_house
      : D1 [0 4]
      : D2 [0 4]
      : D3 [0 4]
      : D4 [0 4]
      : S0 [0 4] # smoke_brand_of_first_house
      : S1 [0 4]
      : S2 [0 4]
      : S3 [0 4]
      : S4 [0 4]
      : P0 [0 4] # pet_of_first_house
      : P1 [0 4]
      : P2 [0 4]
      : P3 [0 4]
      : P4 [0 4]
    ...
    // Solution count
    expect(solver.solutions).toHaveLength(1);
