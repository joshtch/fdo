...
function space_initializeUnsolvedVars(space, config) {
  ...
  if (targetVarNames === 'all') {
    ...
    ASSERT(targetVarNames instanceof Array, 'expecting targetVarNames to be an array or the string `all`', targetVarNames);
...
function space_propagateAll(space, config, propagators, changedVars, changedTrie, cycleIndex) {
  ASSERT_LOG(LOG_FLAG_PROPSTEPS, log => log('space_propagateAll (' + propagators.length + ' propas have changed vars)'));
...
function space_propagateByIndexes(space, config, propagators, propagatorIndexes, changedVars, changedTrie, cycleIndex) {
  ASSERT_LOG(LOG_FLAG_PROPSTEPS, log => log('space_propagateByIndexes (' + propagatorIndexes.length + ' propas have changed vars)'));
  for (let i = 0, n = propagatorIndexes.length; i < n; i++) {
    ...
    ASSERT_LOG(LOG_FLAG_PROPSTEPS, log => log(' - ', (i + 1) + '/' + propagatorIndexes.length, '; prop index=', propagatorIndex, ', prop=', JSON.stringify(propagator).replace(/\n/g, '; ')));
    let rejected = space_propagateStep(space, config, propagator, changedVars, changedTrie, cycleIndex);
    if (rejected) {
      ASSERT_LOG(LOG_FLAG_PROPSTEPS, log => log(' - ', (i + 1) + '/' + propagatorIndexes.length, '; has rejected'));
      return true;
    }
  }
  return false;
...
function space_propagateChanges(space, config, allPropagators, minimal, targetVars, changedVars, changedTrie, cycleIndex) {
  ASSERT_LOG(LOG_FLAG_PROPSTEPS, log => log('space_propagateChanges (' + targetVars.length + ' vars to check), var indexes;', targetVars.slice(0, 10) + (targetVars.length > 10 ? '... and ' + (targetVars.length - 10) + ' more' : '')));
  ASSERT(space._class === '$space', 'EXPECTING_SPACE');
  ASSERT(config._class === '$config', 'EXPECTING_CONFIG');

  let varToPropagators = config._varToPropagators;
  for (let i = 0, vlen = targetVars.length; i < vlen; i++) {
    let varIndex = targetVars[i];
    let propagatorIndexes = varToPropagators[varIndex];
    ASSERT_LOG(LOG_FLAG_PROPSTEPS, log => log(' - var ' + (i + 1) + '/' + targetVars.length, ', varIndex', targetVars[i], ', is part of', propagatorIndexes.length, 'propas;', propagatorIndexes));
...
function space_onBeforePropagate(space, config) {
  let callback = config.beforeSpace;
...
function space_onAfterPropagate(space, config) {
  let callback = config.afterSpace;
  if (callback && callback(space)) {
    config.aborted = true;
    return true;
  }
